...
function buildSummary(data: DashboardData): string[] {
  ...
  if (totalDays === 0) {
    return ["Polter is still learning your patterns. Come back after a few work sessions."];
  }

  const totalActive = data.days.reduce((s, d) => s + d.total_active_minutes, 0);
  lines.push(`Over the past ${totalDays} day${totalDays !== 1 ? "s" : ""}, Polter has observed ${fmtMinutes(totalActive)} of active computer use.`);
  ...
  if (insightCount > 0) {
    lines.push(`Polter has shared ${insightCount} insight${insightCount !== 1 ? "s" : ""} in the last 20 sessions.`);
  }
...
export default function WhatPolterKnows({ data }: { data: DashboardData | null }) {
